...
| 子系统 | 使用者 | 说明 |
|--------|--------|------|
| Cookie 持久化 (`~/.config/boss_zhipin_cookies.json`) | scraper + greeter | Cookie 一次登录，两个模块复用，避免重复扫码 |
...
**能力**：
- **多关键词拓展搜索**：自动生成近义词组合拓宽覆盖面，避免单一关键词遗漏
- **触底滚动翻页**：自动滚动加载列表页所有卡片，非固定页数
- **全局去重**：按 `link` 字段跨关键词去重，同一岗位多次命中只保留一次
- **渐进式限流恢复**：详情页遇到限流自动递增等待时间（指数退避 + 随机抖动）
...
**关键设计**：
- **多规则命中策略**：所有规则都执行、都记录原因，但只计一次过滤。避免单条记录被重复计入 rejected 统计
- **空值安全**：字段为空时对应规则不触发，避免误杀。例如：融资信息缺失 → 跳过"微型初创"检查，而不是拒绝
...
**匹配标准**：
- **完全匹配** (full match)：技能对齐 ≥80%，经验匹配，建议投递
- **部分匹配** (partial match)：部分技能对齐，可尝试投递，最终由用户决定
- **完全不匹配** (no match)：技能不相关，建议跳过

**能力**：
- **批量3并发** LLM 调用加速匹配
- **交互式 HTML 确认页面**：薪资滑块过滤 + 分页浏览 + 批量 accept/reject 操作
- **简历关键信息提取**：自动提取个人总结、项目经验、技能关键词
...
**安全机制**：
- **登录态检测**：Cookie 复用，scraper 登录过的会话直接复用
- **风控识别**：检测每日限额警告、安全验证码、异常页面，自动停止
- **历史去重**：持久化记录到 `greet_history.json`，同一岗位不重复打招呼
- **速率控制**：随机间隔 + 人工行为模拟，避免触发频率限制
...
**分析维度**：
- **jieba 自动关键词提取**：JD 分词 + 词频统计 + 关键词云
- **动态雷达维度**：24候选池按岗位类型自动选8个最相关维度
- **P75/P25 动态薪资阈值**：按实际数据分布计算，替代固定分位数，适配不同岗位类型
- **高薪 vs 入门技能双列对比**：直观展示不同薪资段的核心技能差异
...
│   └── resume.md                       # 用户简历
└── graphify-out/                       # 知识图谱输出
    ├── graph.html                      # 交互式图可视化（浏览器打开）
    ├── graph.json                      # 原始图数据（164 节点 · 260 边 · 25 社区）
...
- `graphify-out/graph.html` — 交互式图可视化（浏览器打开即可探索模块关系）
- `graphify-out/graph.json` — 原始图数据（164 节点 · 260 边 · 25 社区）
- `graphify-out/GRAPH_REPORT.md` — 审计报告（含 God Nodes、惊喜连接、建议问题）

运行 `graphify query "你的问题"` 对代码库进行自然语言查询。例如：

```bash
# 追踪 process_single_job 跨越了哪些社区
graphify query "process_single_job 连接了哪些模块？"

# 找 scraper 和 greeter 的共享依赖
graphify path "ohmyboss-scrape" "ohmyboss-greeter"
```
